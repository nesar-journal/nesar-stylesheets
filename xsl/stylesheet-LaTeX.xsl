<xsl:stylesheet version="2.0" 
  ...
  <xsl:strip-space elements="tei:p tei:s tei:list tei:item tei:head tei:note tei:div tei:body tei:text tei:TEI tei:lg tei:quote"/>
...
\end{pullquote}
    ...
    <xsl:template match="tei:table">
      <xsl:text>\begin{tabular}{llllllllllllllll}\toprule
</xsl:text>
      <xsl:choose>
...
\end{tabular}
    ...
    <xsl:template match="tei:text">
      <xsl:text>\thispagestyle{firstpage}</xsl:text>
      <xsl:apply-templates/>
    </xsl:template>
